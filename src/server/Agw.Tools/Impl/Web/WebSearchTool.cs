...
internal sealed class WebSearchTool
{
        ...
        List<string>? blockedDomains)
    {
        ...
        var html = SendSearchRequest(
            client,
            "Google",
            searchUrl,
            "en-US,en;q=0.9");

        if (Regex.IsMatch(html, @"unusual traffic|detected unusual traffic|sorry/index|To continue, please type", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
        {
        ...
        List<string>? blockedDomains)
    {
        ...
        return EnsureResults(
            "Bing",
            ExtractBingResults(html, maxResults),
            maxResults,
            allowedDomains,
            blockedDomains);
    }
        ...
        List<string>? blockedDomains)
    {
        ...
        if (Regex.IsMatch(html, @"百度安全验证|网络不给力|请输入验证码|verify", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
        {
    ...
    private static string SendSearchRequest(
        HttpClient client,
        string provider,
        string searchUrl,
    ...
    private static List<SearchHit> ExtractGoogleResults(string html, int maxResults)
    {
        ...
        for (var index = 0; index < headings.Count && results.Count < maxResults; index++)
        {
            ...
            if (string.IsNullOrWhiteSpace(title) || string.IsNullOrWhiteSpace(url) || url.Contains("/search?", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
    ...
    private static string ResolveSearchResultUrl(SearchProvider provider, string rawUrl)
    {
        ...
        if (provider == SearchProvider.Google)
        {
            try
            {
                ...
                var target = ParseQueryParameter(url.Query.TrimStart('?'), "q")
        ...
        {
            var baseUrl = provider == SearchProvider.Bing
                ? "https://www.bing.com"
                : "https://www.baidu.com";
