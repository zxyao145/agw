...
public class AgentRuntimeFactoryCompositionTests
{
    [Fact]
    ...
    {
        var instructions = AgentRuntimeFactoryUtil.BuildInstructions("System prompt");
    ...
    {
        var instructions = AgentRuntimeFactoryUtil.BuildInstructions("   ");
    ...
    public void MergeEnvironmentVariables_ExecutionValuesOverrideAgentDefinitionValues()
    {
        ...
        var result = AgentRuntimeFactoryUtil.MergeEnvironmentVariables(agentVariables, executionVariables);
    ...
    public void MergeEnvironmentVariables_ProjectAndExecutionValuesOverrideEarlierLayers()
    {
        ...
        var result = AgentRuntimeFactoryUtil.MergeEnvironmentVariables(
            agentVariables,
            projectVariables,
            executionVariables
        );
    ...
    public void ExternalAgentNames_Codex_HasDefaultCodexOptions()
    {
        ...
        Assert.Equal(EngineKind.Codex, codexAgent.ExternalAgentKind);
    ...
    [Theory]
    [InlineData(EngineKind.ClaudeCode, "custom-claude")]
    [InlineData(EngineKind.Codex, "custom-codex")]
    [InlineData(EngineKind.Pi, "custom-pi")]
    public void EngineKinds_ExternalAgent_UsesPersistedKindRegardlessOfName(EngineKind externalAgentKind, string name)
    {
        // Arrange
        var agent = new Agent
        {
    ...
    [Theory]
    [InlineData(AgentType.System, true, true)]
    [InlineData(AgentType.System, false, true)]
    [InlineData(AgentType.External, false, false)]
    public void ResolveSummaryModelProviderId_MissingSummaryModel_FallsBackOnlyForSystemAgents(
        AgentType type,
        bool configured,
        bool expected
    )
    {
        var agent = new Agent
        {
            ...
            SummaryModelProviderId = configured ? Guid.CreateVersion7() : null,
        };

        var resolved = AgentRuntimeFactory.ResolveSummaryModelProviderId(agent);

        Assert.Equal(expected ? agent.SummaryModelProviderId ?? agent.ModelProviderId : null, resolved);
    }

    [Fact]
    public void EngineKinds_SystemAgent_ReturnsMaf()
    {
        var systemAgent = new Agent { Type = AgentType.System, ExternalAgentKind = EngineKind.Pi };

        Assert.Equal(EngineKind.Maf, EngineKinds.Resolve(systemAgent));
    }

    [Fact]
    public void EngineKinds_UnsupportedExternalKind_ThrowsUnsupportedAgentType()
    {
        var unknownExternalAgent = new Agent { Type = AgentType.External, ExternalAgentKind = (EngineKind)99 };

        var exception = Assert.Throws<AgwException>(() => EngineKinds.Resolve(unknownExternalAgent));

        Assert.Equal(ErrorCodes.UnsupportedAgentType.Code, exception.Code);
    }

    [Fact]
    public void WrapExternalAgent_UsesHistoryRecordingWithoutCreatingSdkAgent()
    {
        var service = CreateRuntimeFactory();

        var agent = service.WrapExternalAgent(new StubAIAgent(), isBackground: false);

        Assert.NotNull(agent.GetService<HistoryRecordingAgent>());
    ...
    public async Task WrapExternalAgent_UserMemory_LogsOriginalRequestAndInjectsMemoryOnlyIntoSdk()
    {
        ...
        var service = CreateRuntimeFactory(fixture.Store, logger);
    ...
    [Fact]
    public void WrapClaudeCodeAgent_DecoratesWithSessionTrackerAndHistoryRecording()
    {
        var service = CreateRuntimeFactory();
    ...
    [Fact]
    public void WrapPiAgent_DecoratesWithHistoryRecordingWithoutSessionTracker()
    {
        var service = CreateRuntimeFactory();
    ...
    public void BuildPiAgentAIAgentOptions_PreservesAgentExtraAndOverridesRuntimeState()
    {
        ...
        // Act
        var options = AgentRuntimeFactory.BuildPiAgentAIAgentOptions(
    ...
    public void DisableExternalSdkChatHistoryPersistence_ForCodex_ClearsProvider()
    {
        ...
        var codexOptions = AgentRuntimeFactory.DisableExternalSdkChatHistoryPersistence(
    ...
    public void UsesProviderSessionBinding_SupportsClaudeCodeCodexAndPiExternalAgents()
    {
        ...
        Assert.Equal(
            ErrorCodes.UnsupportedAgentType.Code,
            Assert
                .Throws<AgwException>(() =>
                    ExternalProviderSessionBindings.UsesProviderSessionBinding(
                        new Agent { Type = AgentType.External, ExternalAgentKind = EngineKind.Maf }
                    )
                )
                .Code
        );
        Assert.False(
            ExternalProviderSessionBindings.UsesProviderSessionBinding(
                new Agent { Type = AgentType.System, ExternalAgentKind = EngineKind.ClaudeCode }
            )
        );
    }
    ...
    /// <summary>
    /// 只验证装饰结构的测试使用没有数据库的真实历史存储；运行 Agent 的测试传入测试数据库的存储。
    /// Tests that only check decoration use a real history store without a database; tests that run the Agent pass the test database's store.
    /// </summary>
    private static AgentRuntimeFactory CreateRuntimeFactory(
