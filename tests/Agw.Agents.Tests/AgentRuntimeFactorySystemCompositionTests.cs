...
public class AgentRuntimeFactorySystemCompositionTests
{
    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    ...
    )
    {
        ...
        var runtimeService = CreateRuntimeFactory(
            agentAppService,
        ...
        try
        {
            ...
            var recordingAgent = aiAgent!.GetService<Agw.Agents.Execution.Agents.History.HistoryRecordingAgent>();
            Assert.NotNull(recordingAgent);
            var memoryFactoryField = typeof(Agw.Agents.Execution.Agents.History.HistoryRecordingAgent).GetField(
                "_createMemoryContextAsync",
                BindingFlags.Instance | BindingFlags.NonPublic
            );
            Assert.NotNull(memoryFactoryField);
            var memoryFactory = memoryFactoryField.GetValue(recordingAgent);
            if (includeUserMemory)
            {
            ...
            var approvalAgent = Assert.IsType<MafApprovalBatchAgent>(aiAgent!.GetService<MafApprovalBatchAgent>());
            var rulesField = typeof(MafApprovalBatchAgent).GetField(
                "_autoApprovalRules",
                BindingFlags.Instance | BindingFlags.NonPublic
            );
            ...
            Assert.True(await ApprovesAsync(approvalRules, new FunctionCallContent("read", "load_skill")));
            Assert.False(await ApprovesAsync(approvalRules, new FunctionCallContent("write", "run_shell")));
            Assert.DoesNotContain(ToolApprovalAgent.AllToolsAutoApprovalRule, approvalRules);

            async Task<bool> ApprovesAsync(
                IReadOnlyList<Func<ToolAutoApprovalRuleContext, ValueTask<bool>>> rules,
                FunctionCallContent call
            )
            {
                foreach (var candidate in rules)
                    if (await candidate(new ToolAutoApprovalRuleContext(call, aiAgent!, null, [], null)))
                        return true;
                return false;
            }
    ...
    )
    {
        return new AgentRuntimeFactory(
            ...
            new Agw.Agents.Execution.Agents.History.HistorySessionState(),
