...
public sealed class ApiTokenAppServiceTests : IDisposable
{
    ...
    public async Task CreateTokenAsync_WhenNameIsBlankOrTooLong_ThrowsInvalidParamWithoutPersisting(string name)
    {
        await using var fixture = await TokenStoreFixture.CreateAsync();

        var exception = await Assert.ThrowsAsync<AgwException>(() =>
            fixture.Store.CreateTokenAsync(name, TestContext.Current.CancellationToken)
        );
    ...
    private sealed class TokenStoreFixture : IAsyncDisposable
    {
        private TokenStoreFixture(SqliteConnection connection, AgwDbContext context)
        {
            ...
            Context = context;
            Store = TestApiTokenStore.Create(context);
        }

        public SqliteConnection Connection { get; }
        public AgwDbContext Context { get; }
        public ApiTokenAppService Store { get; }
