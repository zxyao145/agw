...
A typical write chain is `Controller -> AppService / RuntimeService -> new XxxBehavior(completeData) -> Behavior method -> I<Module>DbContext / internal persistence adapter -> EF Core`.
...
Persisted Agentflows are Input-rooted directed graphs compiled into Microsoft Agent Framework workflows by `AgentflowWorkflowCompiler`. They support Direct, Fan Out, ordered Switch, Fan-in Barrier, orchestration-block, HumanGate, and controlled-cycle semantics. The current `AgentflowDomainService` validates graph structure, references, block membership, routing-strategy consistency, cycle exits, and cyclic barrier constraints before persistence; it is legacy migration debt for the repository-wide `<Entity>Behavior` pattern. See [Agentflow Graphs, Editor, and Chat Attribution](6.Agentflow.md) for the complete graph contract and client behavior.
...
Application loads the complete data boundary and external facts, manually constructs the Behavior with `new`, invokes its domain methods, and persists the resulting data. Behavior may mutate only the bound root and its owned children. It is not an IoC service and cannot depend on persistence, transport, current-user, MAF/MCP, or other Infrastructure concerns. Simple CRUD does not require a Behavior.

Pure Domain Policy is manually constructed at its call site by default. A genuine DomainService whose rule spans multiple data boundaries may be managed by IoC, but it must remain stateless and may depend only on pure Domain components. A single-root `XxxDomainService` is misplaced Behavior; Application supplies data and external facts to cross-boundary DomainService methods and applies their decisions through the corresponding Behaviors.
...
Owns persistence primitives and continues to host entity CLR types and EF configurations during the ownership migration. The ownership manifest assigns every table to one Module; the physical location in Agw.Data does not grant cross-module query or write access.
