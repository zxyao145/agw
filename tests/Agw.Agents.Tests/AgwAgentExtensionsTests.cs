...
public sealed class AgwAgentExtensionsTests
{
    ...
    public async Task AsAgwAgent_CompactionProvider_RunsForEveryModelCallAndSetsOutputLimit()
    {
        ...
        await agent.RunAsync(requestMessages, session, cancellationToken: TestContext.Current.CancellationToken);

        Assert.Equal(2, strategy.InvocationCount);
        Assert.Equal([64_000, 64_000], client.MaxOutputTokens);
        Assert.All(
            requestMessages,
            message => Assert.Equal(AgentRequestMessageSourceType.External, message.GetAgentRequestMessageSourceType())
        );
    ...
    public async Task AsAgwAgent_StreamingCompaction_PreservesFunctionLoopContextWithoutDuplication()
    {
        ...
        var agent = client.AsAgwAgent(
            CreateDefinition(
                new InMemoryChatHistoryProvider(),
                new CompactionProvider(new CountingCompactionStrategy(), stateKey: "streaming-compaction")
            ),
            CreateCapabilities(tools: [function]),
            NullLoggerFactory.Instance,
            new ServiceCollection().BuildServiceProvider()
        ...
        var session = await agent.CreateSessionAsync(TestContext.Current.CancellationToken);
        var requestMessage = new ChatMessage(ChatRole.User, "search").WithAgentRequestMessageSource(
    ...
    public async Task ContextWindowCompactionStrategy_LongHistory_PreservesSystemAndRecentMessages()
    {
        ...
        for (var turn = 0; turn < 40; turn++)
        {
            ...
            if (turn == 2)
            {
                ...
                messages.Add(
                    new ChatMessage(ChatRole.Tool, [new FunctionResultContent(toolCallId, new string('r', 1_000))])
                );
            }
    ...
    public async Task AsAgwAgent_AutoApprovedFunctionLoop_PreservesHistoryAndDeduplicatesContext()
    {
        ...
        var client = new FunctionCallingStubChatClient(functionCallCount: 2, functionRoundCount: 3);
        var agent = client.AsAgwAgent(
            CreateDefinition(
                historyProvider,
        ...
        var originalMessages = Enumerable
            ...
            .ToList();
        await foreach (
            var _ in agent.RunStreamingAsync(originalMessages, session, cancellationToken: cancellationToken)
        ) { }
        ...
        Assert.Contains(modelRequest, message => message.Text.StartsWith("user-0:", StringComparison.Ordinal));
        Assert.Contains(modelRequest, message => message.Text.StartsWith("user-39:", StringComparison.Ordinal));
        ...
        var storedMessages = records
            ...
            .ToList();
        Assert.Contains(storedMessages, message => message.Text.StartsWith("user-0:", StringComparison.Ordinal));
        Assert.Contains(storedMessages, message => message.Text.StartsWith("user-39:", StringComparison.Ordinal));
        Assert.Single(
            storedMessages,
            message => message.Text.StartsWith("### Current todo list", StringComparison.Ordinal)
        );
        ...
        Assert.Equal(6, storedMessages.SelectMany(message => message.Contents).OfType<FunctionCallContent>().Count());
    ...
    public async Task AsAgwAgent_StalledCompactionState_RebuildsFromCompleteHistory()
    {
        ...
        var agent = client.AsAgwAgent(
            CreateDefinition(
                historyProvider,
                new CompactionProvider(new CountingCompactionStrategy(), stateKey: "stalled-compaction")
            ),
            CreateCapabilities(contextProviders: [new StaticTextContextProvider("memory context")]),
            NullLoggerFactory.Instance,
            new ServiceCollection().BuildServiceProvider()
        );
        var session = await agent.CreateSessionAsync(cancellationToken);
    ...
    public async Task AsAgwAgent_PlanModeModelRequestsPnpmFmt_DoesNotInvokeShellOrRequestApproval()
    {
        ...
        var planModeResult = Assert.Single(
            response.Messages.SelectMany(static message => message.Contents).OfType<FunctionResultContent>()
        );
    ...
    public async Task AsAgwAgent_ApprovedExecuteToolAfterSwitchingToPlan_DoesNotInvokeTool()
    {
        ...
        var modeProvider = new AgentModeProvider(new AgentModeProviderOptions { DefaultMode = "execute" });
        ...
        var approvalRequest = Assert.Single(
            firstResponse.Messages.SelectMany(static message => message.Contents).OfType<ToolApprovalRequestContent>()
        );

        await modeProvider.SetModeAsync(session, "plan", TestContext.Current.CancellationToken);
        ...
        var result = Assert.Single(
            secondResponse.Messages.SelectMany(static message => message.Contents).OfType<FunctionResultContent>()
        );
    ...
    public async Task AsAgwAgent_InvocationWarning_EmitsOnlyAfterToolResult()
    {
        ...
        var agent = new FunctionCallingStubChatClient().AsAgwAgent(
            CreateDefinition(),
            CreateCapabilities(
                tools: [tool],
                toolInvocationWarnings: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
                {
        ...
        Assert.Contains(updates.SelectMany(update => update.Contents), content => content is FunctionResultContent);
    }
    ...
    public async Task AsAgwAgent_ApprovalContinuation_WhenToolFails_KeepsFunctionResultNextToFunctionCall()
    {
        ...
        var functionResult = Assert.Single(secondRequest[functionResultIndex].Contents.OfType<FunctionResultContent>());
        var functionCall = Assert.Single(secondRequest[functionResultIndex - 1].Contents.OfType<FunctionCallContent>());
        ...
        var storedFunctionCallIndex = storedMessages.FindIndex(message =>
            message.Contents.OfType<FunctionCallContent>().Any(content => content.CallId == functionCall.CallId)
        );
        var storedFunctionResultIndex = storedMessages.FindIndex(message =>
            message.Contents.OfType<FunctionResultContent>().Any(content => content.CallId == functionCall.CallId)
        );
    ...
    private static void AssertTodoSnapshots(IReadOnlyList<AgentResponseUpdate> updates)
    {
        ...
        foreach (var snapshot in snapshots)
        {
            ...
            var result = Assert.Single(updates[snapshot.Index - 1].Contents.OfType<FunctionResultContent>());
            Assert.Equal(result.CallId, snapshot.Update.AdditionalProperties!["callId"]?.ToString());
    ...
    private static ChatMessage CreateFunctionCallMessage(string callId) =>
        new(ChatRole.Assistant, [new FunctionCallContent(callId, "web_search", new Dictionary<string, object?>())]);
    ...
    private sealed class FunctionCallingStubChatClient : IChatClient
    {
        ...
        )
        {
            ...
            return Task.FromResult(
                completedRounds >= _functionRoundCount
    ...
    private sealed class MutableChatHistoryProvider : ChatHistoryProvider
    {
        ...
        protected override ValueTask<IEnumerable<ChatMessage>> ProvideChatHistoryAsync(
            InvokingContext context,
            CancellationToken cancellationToken = default
        ) => ValueTask.FromResult<IEnumerable<ChatMessage>>(Messages.Select(static message => message.Clone()));
    }
    ...
    private sealed class IllegalShellCallChatClient : IChatClient
    {
        ...
        private static ChatMessage CreateResponse(IEnumerable<ChatMessage> messages) =>
            messages.SelectMany(static message => message.Contents).OfType<FunctionResultContent>().Any()
    ...
    private sealed class ApprovalShellCallChatClient : IChatClient
    {
        ...
        private static ChatMessage CreateResponse(IEnumerable<ChatMessage> messages) =>
            messages.SelectMany(static message => message.Contents).OfType<FunctionResultContent>().Any()
                ...
                : new ChatMessage(
                    ChatRole.Assistant,
                    [new FunctionCallContent("format-call", "run_shell", new Dictionary<string, object?>())]
                );
    }
        ...
        private static ChatMessage CreateResponse(IEnumerable<ChatMessage> messages)
        {
            ...
            return latestCallId switch
            {
                null => CreateFunctionCall(
                    ...
                    new Dictionary<string, object?>
                    {
                        ["todos"] = JsonSerializer.SerializeToElement(
                            new[]
                            {
                ...
                "todo-add-call" => CreateFunctionCall(
                    ...
                    new Dictionary<string, object?>
                    {
                        ["items"] = JsonSerializer.SerializeToElement(new[] { new { id = 1, reason = "finished" } }),
                    }
                ),
