...
internal static class AgwA2ADiagnostics
{
    ...
    internal static readonly Histogram<double> RequestDuration = Meter.CreateHistogram<double>(
        "a2a.server.request.duration",
        "ms",
        "Duration of A2A request processing"
    );

    internal static readonly Counter<long> ErrorCount = Meter.CreateCounter<long>(
        "a2a.server.error.count",
        description: "Number of A2A errors"
    );

    internal static readonly Counter<long> TaskCreatedCount = Meter.CreateCounter<long>(
        "a2a.server.task.created",
        description: "Number of tasks created"
    );

    internal static readonly Histogram<double> StreamEventCount = Meter.CreateHistogram<double>(
        "a2a.server.stream.event.count",
        description: "Events per streaming request"
    );

    // ─── Client Metrics ───

    internal static readonly Counter<long> ClientRequestCount = Meter.CreateCounter<long>(
        "a2a.client.request.count",
        description: "Number of A2A client requests sent"
    );

    internal static readonly Histogram<double> ClientRequestDuration = Meter.CreateHistogram<double>(
        "a2a.client.request.duration",
        "ms",
        "Duration of A2A client request processing"
    );

    internal static readonly Counter<long> ClientErrorCount = Meter.CreateCounter<long>(
        "a2a.client.error.count",
        description: "Number of A2A client errors"
    );

    internal static readonly Histogram<double> ClientStreamEventCount = Meter.CreateHistogram<double>(
        "a2a.client.stream.event.count",
