...
public partial class AgentflowTurnExecutorTests : IDisposable
{
    private readonly IDisposable _userScope;
    private readonly IDisposable _executionScope;

    public AgentflowTurnExecutorTests()
    {
        _userScope = UserInfoUtil.Push(
            new ClaimsPrincipal(new ClaimsIdentity([new Claim(ClaimTypes.NameIdentifier, "tester")], "test"))
        );
        // 进程内执行由 Host 建立执行作用域；Durable 分段在各自的分段作用域中执行。
        // In-process execution runs inside the Host execution scope; durable segments use their own segment scopes.
        _executionScope = ExecutionTestScopes
            .Scope(ExecutionTestScopes.Context(userId: "tester", runtimeType: AgentRuntimeType.Agentflow))
            .Push();
    }
    ...
    public async Task ExecuteStreamingAsync_MissingAgentflow_ReportsNotFoundInsteadOfCompletingSilently()
    {
        var service = CreateTestHost(
            ...
            new StubAgentRuntimeFactory(_ => null),
    ...
    public async Task ExecuteAsync_ToolApprovalRequest_FailsUnattendedExecution()
    {
        ...
        var service = CreateTestHost(
            ...
            new StubAgentRuntimeFactory(_ => new ApprovalRequestAgent()),
            new StubProviderSessionState(),
            new RecordingSummaryService()
        );
    ...
    public async Task GetMermaidAsync_AgentCreationFails_DisposesAlreadyCreatedAgents()
    {
        ...
        var service = CreateTestHost(
            ...
            new TestRepository<AgentflowNode>(nodes, item => (item.AgentflowId, item.NodeId)),
            new TestRepository<AgentflowEdge>(edges, item => (item.AgentflowId, item.EdgeId)),
            new StubAgentRuntimeFactory(agentId => agentId == firstAgentId ? firstAgent : null),
            new StubProviderSessionState(),
            new RecordingSummaryService()
        );
    ...
    public async Task ExecuteStreamingAsync_CheckpointMarker_EmitsNamedMafCheckpoint()
    {
        ...
        var logger = new CapturingLogger<AgentflowCheckpointSupport>();
        var service = CreateTestHost(
            logger,
            new TestRepository<Agentflow>([agentflow], item => item.Id),
            new TestRepository<AgentflowNode>(nodes, item => (item.AgentflowId, item.NodeId)),
            new TestRepository<AgentflowEdge>(edges, item => (item.AgentflowId, item.EdgeId)),
            new StubAgentRuntimeFactory(agentId),
            new StubProviderSessionState(),
            new RecordingSummaryService()
        );
    ...
    public async Task ExecuteStreamingAsync_CheckpointAfterToolApproval_EmitsOnce()
    {
        ...
        var service = CreateTestHost(
            ...
            new TestRepository<AgentflowNode>(nodes, item => (item.AgentflowId, item.NodeId)),
            new TestRepository<AgentflowEdge>(edges, item => (item.AgentflowId, item.EdgeId)),
            new StubAgentRuntimeFactory(_ => worker),
            new StubProviderSessionState(),
            new RecordingSummaryService()
        );
    ...
    [Fact]
    public async Task RunAsync_RuntimeSettings_ForwardsSessionEnvironmentVariables()
    {
        var agentflow = new Agentflow
        {
            CreateBy = "tester",
        ...
        var agentRuntimeFactory = new StubAgentRuntimeFactory(agentId);
        ...
        Assert.NotNull(agentRuntimeFactory.LastEnvironmentVariables);
        Assert.Equal("session", agentRuntimeFactory.LastEnvironmentVariables["SESSION_ONLY"]);
        Assert.Equal(conversationId, agentRuntimeFactory.LastConversationId);
        Assert.All(agentRuntimeFactory.CreatedAgents, agent => Assert.True(agent.Disposed));
    }
    ...
    public async Task GetMermaidAsync_NestedWorkflow_DisposesNestedAgents()
    {
        ...
        var agentRuntimeService = new StubAgentRuntimeFactory(agentId);
        var service = CreateTestHost(
            NullLogger<AgentflowCheckpointSupport>.Instance,
    ...
    public async Task ExecuteStreamingAsync_TodoToolBlock_WithoutToolInvocation_DoesNotPersistStateSnapshot()
    {
        ...
        var service = CreateTestHost(
            ...
            new TestRepository<AgentflowNode>(nodes, item => (item.AgentflowId, item.NodeId)),
            new TestRepository<AgentflowEdge>(edges, item => (item.AgentflowId, item.EdgeId)),
            new StubAgentRuntimeFactory(_ => new TrackingAIAgent(enableTodo: true)),
            new StubProviderSessionState(),
            new RecordingSummaryService(),
    ...
    private static AgentflowTurnTestHost CreateTestHost(
    ...
    {
        var runtimeFactory = new AgentflowRuntimeFactory(
        ...
        var executor = new AgentflowTurnExecutor(
            runtimeFactory,
            new AgentflowCheckpointSupport(logger, checkpointStore),
            NullLogger<AgentflowTurnExecutor>.Instance
        );
        return new AgentflowTurnTestHost(runtimeFactory, executor);
    }
    ...
    private sealed class StubAgentRuntimeFactory : IAgentRuntimeFactory
    {
