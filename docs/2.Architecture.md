...
Agw.Setup/             # First-run setup and combined server-state persistence adapter
```
...
Persisted Agentflows are DAGs compiled into Microsoft Agent Framework workflows by `AgentflowWorkflowCompiler`. Supported node kinds are Input, Output, Agent, Workflow-as-Agent, PromptAdapter, HumanGate, CheckpointMarker, Concurrent, Handoff, GroupChat, and Magentic blocks; edges use Direct, FanOut, or FanIn semantics. `AgentflowDomainService` validates graph structure, references, and block membership before persistence.
