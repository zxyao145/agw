...
Web 客户端会把 `/api/*` 代理到后端；默认后端地址是 `http://localhost:30816`。
...
```bash
curl 'http://localhost:30816/api/jobs' \
  --request POST \
...
```bash
curl 'http://localhost:30816/api/jobs/33333333-3333-3333-3333-000000000001/logs' \
  --header 'Authorization: Bearer agw_...'
```
