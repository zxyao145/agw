...
public partial class AgentRuntimeFactory
{
    ...
    )
    {
        ...
        try
        {
            ...
            var historyProvider = CreateHistoryProvider(
                EngineKinds.Resolve(request.Agent),
                structuredResult: hasResponseSchema
            );
            if (
            ...
            if (hasResponseSchema)
            {
                aiAgent = new AgentResponseSchemaExecutionAgent(
                    aiAgent!,
                    "external",
                    EngineKinds.Resolve(request.Agent).ToString(),
    ...
    private bool TryCreateExternalAgent(
        ...
        Project project,
        IReadOnlyDictionary<string, string> environmentVariables,
        AgwChatHistoryProvider historyProvider,
        Func<CancellationToken, ValueTask<ChatMessage?>>? createMemoryContextAsync,
    ...
    )
    {
        ...
        var kind = EngineKinds.Resolve(request.Agent);
        aiAgent = kind switch
        {
            EngineKind.ClaudeCode => CreateClaudeCodeAgent(
                request.Agent,
                project,
            ...
            EngineKind.Codex => CreateCodexAgent(
                request.Agent,
                project,
                request.ProviderSessionId,
                request.IsResume,
        ...
        aiAgent = kind switch
        {
            EngineKind.ClaudeCode => WrapClaudeCodeAgent(
                aiAgent,
                historyProvider,
                isBackground,
                request.OnExternalSessionStartedAsync,
            ...
            EngineKind.Codex => WrapExternalAgent(aiAgent, historyProvider, isBackground, createMemoryContextAsync),
            EngineKind.Pi => aiAgent,
            _ => null,
        };
        return aiAgent != null;
    }
    ...
    internal AIAgent WrapExternalAgent(
        ...
        WrapExternalAgent(
            aiAgent,
            CreateHistoryProvider(EngineKind.Codex, structuredResult: false),
            isBackground,
            createMemoryContextAsync
        );
    ...
    internal AIAgent WrapClaudeCodeAgent(
        ...
        WrapClaudeCodeAgent(
            aiAgent,
            CreateHistoryProvider(EngineKind.ClaudeCode, structuredResult: false),
            isBackground,
    ...
    internal AIAgent WrapPiAgent(
        ...
        DecorateExternalAgent(
            aiAgent,
            CreateHistoryProvider(EngineKind.Pi, structuredResult: false),
            isBackground,
            createMemoryContextAsync
        );
    ...
    {
        var paths = PiRuntimePaths.Create(_dataPaths, UserInfoUtil.RequiredUserId);
        paths.EnsureCreated();
