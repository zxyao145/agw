...
public sealed class PlanModeToolGuardProviderTests
{
    ...
    public async Task InvokingAsync_WithoutSession_FailsClosedAsPlanMode()
    {
        ...
        Assert.Equal(["mode_get", "run_shell"], tools.Select(static tool => tool.Name));
    ...
    public async Task InvokingAsync_PlanMode_FullAccessCannotAutoApproveRestrictedTool()
    {
        ...
        var guardedFunction = Assert.IsType<PlanModeRestrictedAIFunction>(Assert.Single(context.Tools!));
    ...
    public async Task WrappedExecuteTool_AfterSwitchingToPlan_RejectsBeforeInnerInvocation()
    {
        ...
        var session = new TestAgentSession();
        var guard = new PlanModeToolGuardProvider(modeProvider, new HashSet<string>(StringComparer.OrdinalIgnoreCase));
        var executeContext = await InvokeAsync(guard, session, [approvalWrapped]);
        var guardedFunction = Assert.IsType<PlanModeRestrictedAIFunction>(Assert.Single(executeContext.Tools!));
    ...
    public async Task InvokingAsync_ExecuteMode_DuplicateAllowedNameRemainsGuardedAfterPlanSwitch()
    {
        ...
        var context = await InvokeAsync(guard, session, [CreateCountingFunction(), CreateCountingFunction()]);
    ...
    private static async Task<AIContext> InvokeAsync(
        ...
        await provider.InvokingAsync(
            new AIContextProvider.InvokingContext(new TestAgent(), session, new AIContext { Tools = tools }),
            TestContext.Current.CancellationToken
        );
