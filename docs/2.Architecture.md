...
`Agw.Agents.Execution` is the execution implementation assembly of the logical Agents module. It references `Agw.Agents` one-way for definition reads, shared `AgentflowTopology`, and persistence seams. Host explicitly composes `AddAgents` with `AddAgentExecution`; A2A and Jobs continue to use `Agw.Agents.Contracts`. Management requests for Mermaid use the read-only `AgentflowMermaidProvider` through `IAgentflowMermaidProvider`. It uses the execution compiler with descriptive nodes and owner checks without creating external Agents or sessions. Control Plane registers distributed submission and diagrams; Data Plane and Standalone additionally register runtimes and workers.

Execution code is organized by capability. `Inbound` groups SignalR entry points, connection lifecycle, and A2A/Jobs Facades; `Commands` retains the command slices. `Outbound` owns message sending through the shared sink interface, SignalR callback contract and sink, and durable stream sink. `HumanInteraction` groups shared approval contracts and policies with `InProcess` waiting and `Durable` response restoration. `Runtimes` owns shared lifecycle and provider-specific coordination, `Messaging` owns message conversion and durable event storage/replay, and `Persistence/Durable` owns durable state storage. Agent-specific runtimes, adapters, sessions, and runners live under `Agents`; workflow compilation, node context, runners, and checkpoints live under `Agentflows`. Directory and namespace changes preserve the existing public Contracts, DI lifetimes, execution protocol, session keys, and persisted JSON formats.

Connection execution starts through the internal `IExecutionStarter` contract with a resolved data request and an acceptance receipt. The connection-owned InProcess implementation retains the reusable runtime and delegates to `RuntimeFactory`; the Durable implementation delegates registration and attachment to `DurableExecutionSession`. Worker polling, claiming, and recovery remain internal to Durable. Acceptance does not mean execution completion. Existing control operations and the A2A/Jobs Facades retain their respective lifetimes. See [Runtimes](../src/server/Agw.Agents.Execution/Runtimes/README.md).

Each new InProcess turn checks an in-memory SHA-256 fingerprint of the current owner's Agent, Project, Model, Provider endpoint, and authentication configuration through owner-module facades, as well as Context and Generation. Credentials are never logged or persisted in the fingerprint. Creation compares configuration before and after materialization; a concurrent edit prevents later reuse. A changed fingerprint rebuilds the runtime and retains the conversation and external provider-session binding. Active turns retain their snapshot. Skill, MCP, and Connection bindings rebuild on each new turn until their owners expose complete external-content revisions; this trades additional construction cost for correct capability refresh. An already-resolved Workspace still requires restart under the file resolver's existing cache contract.

`AgentRuntimeService` composes Agents. `AgentTurnExecutor` owns turn execution, handoff input, and session persistence. `ExternalProviderSessionBindings` owns external session recovery and binding callbacks. Production dependencies are explicit; unsupported permission and approval capabilities fail explicitly. `Runtimes.ExecutionSettings` is independent of `SettingCommand`; inbound and durable adapters preserve permission versions, interaction policy, resume intent, and persisted JSON compatibility.

Job completion is one cross-module unit of work. The Jobs-owned `IJobOutcomeTransaction` is implemented by `Agw.Infrastructure/Jobs/JobOutcomeTransaction`: a per-Job application lock and one database transaction cover Project Task, Job, and JobLog writes. Rollback clears accepted tracked state before retries. The recorder retains ownership checks and duplicate-attempt rejection.

External Agents may bind an optional Model Provider: Claude Code accepts Anthropic, Codex accepts OpenAI Responses, and Pi accepts all three provider types. The owner-scoped provider configuration overrides corresponding Extra/environment settings at runtime without rewriting host SDK configuration; clearing the binding restores the external tool's configuration.

`GET /api/agents/permission-capabilities?type=0&id=<agentId>` returns supported modes and a reason; `type=1` intersects an Agentflow's nested target capabilities. System Agents and Claude Code support FullAccess, AlwaysAsk, and AllowSameArguments; Codex/Pi support only FullAccess. Each turn freezes its permission snapshot. `SetPermissionModeCommand` changes the next turn, and `permission-status` reports active and pending choices. See [the client execution contract](ws-flow.md).

Durable manifest, task snapshot, and settings are data-only persistence types in `Agw.Agents`; runtime mappings stay in Execution. The permission enum lives in Contracts with unchanged JSON names. Both assemblies share Agents data ownership and the single `AgwDbContext`; physical project references and logical ownership are checked separately. External Agent catalog/default settings remain in Agents and still use SDK option types, so this assembly split does not remove the full SDK dependency from Control Plane. See [ADR 0003](adr/0003-agents-execution-assembly.md).
...
`Execution:Provider` selects one process-wide runtime. `InProcess` keeps active turns and Human-in-the-loop state in the Standalone process. `Distributed` persists execution ownership, manifests, checkpoints, pending interactions, and responses in PostgreSQL, uses a PostgreSQL distributed lock for at-least-once segment execution, and replays output through PostgreSQL or Redis. SignalR, distributed Jobs, and distributed A2A share this durable execution interface; Data Plane workers claim the persisted work. Distributed clients use a stable `executionId` and cursor to reattach after disconnects; the authenticated user ID is the ownership and authorization seam. InProcess clients use `FindInProcessExecution(projectId, contextId)` and `RecoverInProcessExecution(connectionId, interrupt)` to discover, poll, or stop a still-active connection owned by the same user. This recovers status and stop capability within the surviving Server process; disconnected output is not replayed.

Definition Agents created through `CreateDefinitionAgentAsync` reserve each model's `MaxOutputTokens` from its `MaxContextWindowTokens`, then run the `ContextWindowCompactionPipeline` before every model call inside the function-invocation loop. The effective input budget is the difference between those limits; at 50% `ToolResultEvictionCompactionStrategy` replaces the result bodies of older Tool call groups with a placeholder while keeping each assistant message's reasoning, calls, and call/result pairing intact, and at 80% MAF's `TruncationCompactionStrategy` truncates older message groups. Keeping assistant output untouched keeps compacted requests valid for providers that validate replayed reasoning, such as DeepSeek thinking mode. Compaction changes only the request view: EF Core retains the original history, while compaction state persists with the Agent session. External Agents and one-shot summary clients do not use this pipeline. See the Execution README for the full boundary and state-key contract.

Persisted Agentflows are Input-rooted directed graphs compiled into Microsoft Agent Framework workflows by `AgentflowWorkflowCompiler`. They support Direct, Fan Out, ordered Switch, Fan-in Barrier, orchestration-block, HumanGate, and controlled-cycle semantics. Agentflow is the current selective-DDD subdomain: `AgentflowConfigurationParser` parses JSON in Application; `AgentflowDefinitionPolicy` evaluates graph data, parsed data-only configuration facts, and external facts into `AgentflowDefinitionDecision`, then `AgentflowBehavior` applies that Decision to the `Agentflow` root and owned Nodes/Edges. `AgentflowTopology` supplies reusable read-only algorithms to Policy, compiler, and runtime. CRUD, checkpoint/history, trace, and durable orchestration stay outside the Behavior. See [Agentflow Graphs, Editor, and Chat Attribution](6.Agentflow.md) for the complete graph contract and client behavior.

Distributed claims carry the persisted `StateVersion` through segment completion. Application lock leases expose `HandleLostToken`; workers cancel on lease loss or a changed claim version and reject stale results before publishing terminal output. This preserves at-least-once execution semantics for external effects.

Conversation history uses execution-scoped buffering owned by Projects. `EfCoreChatHistoryProvider` also owns original-request staging and stream lifecycle state; Agents uses `IConversationHistoryRequests` through Contracts, and the Claude/Pi adapters delegate directly to the EF provider. The Host template uses `ConversationHistory:Mode=Interval` with a ten-second interval (five seconds when the interval key is omitted); `TurnEnd` and `Immediate` are also available. Normal model calls enqueue inputs after reading context and capture streaming deltas before the SDK completion callback. Periodic response snapshots retain stable history IDs; completion replaces them with canonical messages. Model-history reads combine committed and pending records, excluding unfinished response snapshots. SDK session saves and checkpoint boundaries hold a history flush barrier so recovery state cannot advance past uncommitted messages. Distributed output events independently batch at 250 milliseconds or 100 entries, preserving their existing positions and replay cursors. See [conversation persistence configuration](operations/conversation-persistence.md).

Conversation reset increments `ProjectConversation.Generation` and clears history, checkpoint, trace, provider bindings, and SDK session state in one transaction. In-process starts and resets share a conversation execution lease; queued/running/waiting durable executions block reset. Reset-sensitive writes retain their original generation and use `SaveConversationChangesAsync`, whose Infrastructure implementation locks the owned Project before conditionally locking the Conversation and committing children. Connection scopes refresh tracked conversation data and discard cached runtimes after a reset. See [runtime consistency deployment and maintenance](operations/backend-runtime-consistency.md).
...
The eight legacy entity-specific DomainServices have been removed; their architecture allowlist is empty. `AgentBehavior`, `McpServerBehavior`, `ProjectBehavior`, and `ProviderBehavior` retain root invariants. Model token limits, Skill validation/content paths, Project naming/default workspaces, and conversation-history ordering live in neutral Rules. ModelProvider CRUD stays in Application, and no Model/ModelProvider/Skill Behavior is introduced.

Provider updates load AuthConfigs before invoking Behavior. Existing child IDs are reconciled in place; newly requested credentials have no ID until persistence assigns one. The public request still replaces the complete credential set because it does not carry AuthConfig IDs. Local Skill content continues to use `skills/{id:N}`; remote and built-in Skills have no local content path.

Migrated writes load tracked entities and leave creator/modifier stamping to EF interceptors. `EfRepository.Update` preserves the original modification flags of tracked audit properties so ordinary, child-only, and no-op updates refresh root audit metadata while explicit overrides remain intact. Reference validation resolves the authenticated user through the current-user service.
...
Agentflows execute through the same SignalR turn boundary as Agents. Tool approval, workflow gates, and user input use distinct request/response data in `Agw.Agents.Contracts`, projected as `interaction-request` messages. `HumanResponseCommand.response` identifies the interaction by kind and stable ID. InProcess owns register/publish/wait/cleanup in one session; Durable returns explicit Pending results and commits requests with resume state before publishing. Pure permission rules are shared; SDK conversion and grants belong to the MAF adapter. See [HumanInteraction](../src/server/Agw.Agents.Execution/HumanInteraction/README.md) for wiring and extension examples. Agw does not expose a separate Agentflow run or checkpoint REST lifecycle.
...
Owns lightweight Tool declarations (`IAgwToolMeta`, `IAgwTool`, `IProjectScopedAgwTool`), generated-tool contracts, permissions, and declaration attributes under `Agw.Tools.Abstractions.*`. It references only `Microsoft.Extensions.AI.Abstractions`. Contextual/ToolBlock materialization contracts and runtime binding remain in `Agw.Tools`.

Business modules can provide stateless project-scoped Tools through `IAgentSkillRegistration.Tools`. Execution materializes these only for bound Skills, checks names against other capabilities, and applies Tool permissions and Plan-mode declarations. `agw-job` contributes five `agw_job_*` tools; its Skill retains instructions and resources. These tools do not need global catalog entries or persisted Tool definitions.

## Agw.Tools

Owns the unified Tool capability model. `Agw.Tools.Generators` produces attributed Tool metadata, schemas, and direct calls during compilation. Partial Skill and ToolBlock declarations use `IAgwToolSet<T>` for type-safe membership; the generator supplies `ToolTypes` and fixed ToolBlock descriptor factories. At Host startup, `ToolRegistryService` consumes generated modules and discovers only the retained manual `IAgwTool`, `IProjectScopedAgwTool`, `IContextualTool`, and complex `IToolBlock` implementations. A standalone Tool can be selected independently; a ToolBlock is an atomic group whose member Tools are selected and removed together. Every standalone Tool and ToolBlock member declares `AgwToolPermission`; the Agent execution boundary turns `Write` and `Execute` declarations into approval requests while `None` and `ReadOnly` run directly. `ToolContribution` carries contextual Tools, ToolBlock providers, permission metadata, warnings, and owned runtime resources.

See [Tool Design](approachs/tool.md) for the current declaration, generation, registration, composition, and invocation flows.
...
`Agw.Agents.Execution`, outside the Tool catalog.

See [`Agw.Tools/README.md`](../src/server/Agw.Tools/README.md) for the runtime lifecycle and extension guide.

## Agw.Files

`Project.Workspace` supplies the primary directory and default Agent/Shell cwd, retaining the existing default-path rule. Project-owned `AdditionalDirectories` persists stable IDs and paths in a separate JSON column. File/Git references are `(projectId, directoryId?, relativePath)`; omitting directoryId selects the primary root. Additional roots extend the accessible work without changing cwd. Network storage must first be mounted by the OS/container, and all execution nodes must see those host paths.

`ProjectScopedFileSystemResolver` caches `CachedEntry(FileSystem, OwnerUserId, CreatedAt)` by Project ID, directory ID, and normalized path. It rechecks ownership and invalidates local entries when configuration is saved or the Project is deleted; there is no TTL. File panels immediately reload their dropdown selection. Agent turns capture immutable directory snapshots and fingerprints, rebuild changed runtimes on the next turn, and retain conversation/session identities. Child execution and durable manifests use the captured snapshot; legacy manifests gain only a primary root on recovery. Projects supplies configuration through existing provider/runtime contracts; Files does not own or query Project entities.
...
- `ProjectConversation` - Project-scoped conversation identified by its own ID; ContextId provides runtime continuity and Generation invalidates reset-sensitive writes
...
- `ProjectConversationBinding` - External provider session binding for an Agent within a project conversation, stored in `project_conversation_binding`
...
- `DurableExecutionRecord` - User-owned execution manifest, state, checkpoint, pending interactions, and responses; indexed Project/Conversation scope supports lifecycle cleanup without manifest scans. Legacy scopes are backfilled once; unreadable executions are quarantined under the execution lock and concurrency version.
