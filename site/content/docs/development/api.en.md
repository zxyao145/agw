...
`responseSchema` holds the JSON Schema text configured on an agent. It appears in the full agent response: `GET /api/agents/{id}`, `GET /api/agents/paged`, `POST /api/agents`, and `PUT /api/agents/{id}`. The selector endpoint `GET /api/agents` omits it.
...
On update, an absent field keeps the current value, `null` clears it, and a non-empty string replaces it. The server requires valid JSON whose root is an object and otherwise returns an invalid-parameter error. The schema is stored and forwarded as text only.
...
After backend contract changes, run `pnpm gen:api` from `src/clients` and validate callers. Do not hand-edit generated `openapi.d.ts` or log actual API Keys.
