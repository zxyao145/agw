...
public sealed class FileAccessToolBlock : IToolBlock
{
    ...
    )
    {
        ...
        contribution.PlanModeAllowedToolNames.UnionWith(["file_access_read", "file_access_ls", "file_access_grep"]);
        contribution.ContextProviders.Add(
            new FileAccessProvider(new ProjectAgentFileStore(_fileSystemResolver, context.ProjectId))
        );
