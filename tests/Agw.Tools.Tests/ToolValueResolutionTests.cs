...
public sealed class ToolValueResolutionTests
{
    ...
    public void Resolve_ProjectEmpty_PreservesAgentDefinitions()
    {
        ToolValueObject[] agent =
        [
            Tool(new WebSearchToolDefinition()),
            Block(new TodoToolBlockDefinition())
        ];
    ...
    public void Resolve_ProjectValues_AreUnionedWithAgentValues()
    {
        ...
        [
            Tool(new WebFetchToolDefinition()),
            Block(new FileAccessToolBlockDefinition())
        ];
    ...
    public void Resolve_ProjectDefinition_WithSameName_OverridesAgentDefinition()
    {
        ToolValueObject[] agent =
        [
            Block(new ProjectMemoryToolBlockDefinition
            {
                Options = new ProjectMemoryToolBlockOptions
                {
                    Storage = ProjectMemoryStorage.Database
                }
            })
        ];
        ToolValueObject[] project =
        ...
        var definition = Assert.IsType<ProjectMemoryToolBlockDefinition>(
            Assert.Single(resolved.ToolBlocks));
    ...
    public void Resolve_ProjectContainsAgentOnlyDefinition_Throws()
    {
        ...
        var exception = Assert.Throws<AgwException>(
            () => ToolValueResolution.Resolve(null, project));
    ...
    public void Resolve_DuplicateNameWithinOwner_Throws()
    {
        ...
        var exception = Assert.Throws<AgwException>(
            () => ToolValueResolution.Resolve(agent, null));

        Assert.Contains("duplicated", exception.Message);
    }

    private static ToolValue Tool(ToolDefinition definition) =>
        new() { Definition = definition };

    private static ToolBlockValue Block(ToolBlockDefinition definition) =>
        new() { Definition = definition };
}
