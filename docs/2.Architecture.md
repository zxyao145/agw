...
Persisted Agentflows are DAGs compiled into Microsoft Agent Framework workflows by `AgentflowWorkflowCompiler`. Supported node kinds are Input, Output, Agent, Workflow-as-Agent, PromptAdapter, ClearMessages, HumanGate, CheckpointMarker, Concurrent, Handoff, GroupChat, and Magentic blocks. ClearMessages discards the current upstream message payload while continuing the turn with empty input; it does not reset downstream Agent session history. Edge routing supports independent Direct predicates, multi-selection Fan Out predicates, ordered first-match Switch cases with an optional default, and Fan-in Barriers grouped by target. Input is a normal routing source and may also participate in a barrier; `ConfigJson.switchCaseOrder` stores Switch case order without a separate edge-group model. `AgentflowDomainService` validates graph structure, references, block membership, routing-strategy consistency, and Switch invariants before persistence.
