...
`Agw.Agents.Execution.csproj` 是 Agents 逻辑模块的执行实现程序集，单向引用 `Agw.Agents`。Host 分别调用 `AddAgents` 与 `AddAgentExecution`；定义管理通过 Contracts 中的 `IAgentflowMermaidProvider` 查询 Mermaid。启动清单的纯数据结构与持久化接口留在 Agents，运行时映射留在此工程。程序集划分见 [ADR 0003](../../../docs/adr/0003-agents-execution-assembly.md)。
...
Checkpoint 和 Agent Session 的持久化仍经过既有 Application Port / Infrastructure Adapter。Factory 中的 Repository 查询仅访问 Agents 所有的数据；程序集拆分保持既有持久化边界与格式，定义读取仍属于 Agents 逻辑模块。
...
`NormalizedChatHistoryProvider` 在 Execution 内维护每次 producer 调用的消息正文。普通模型、Claude、Codex、Pi 通过 `IAgentMessageAdapter<AgentResponseUpdate>` 提供新增内容，统一按消息和内容块身份追加；Projects 的 `EfCoreChatHistoryProvider` 负责原始输入去重、缓冲、owner/generation 校验和数据库提交。`AgentRequestContextAgent` 暂存原始输入并向 SDK 转发含记忆的临时副本；待写输入按 SDK session 隔离。

普通模型的 `StreamingChatHistoryClient` 位于 MAF 的 per-service-call 历史读取之后。流式内容到达时累积正文，在读取和提交边界序列化完整快照。同一个 MessageId 跨批次、跨 flush 更新同一行；同一来源的捕获和提交串行执行，保留首次排序、行 ID 和输入的任务关联。持久化确认只清除对应捕获对象的变更标记。执行结束、取消和失败时提交已经收到的内容；模型历史根据消息用途和工具调用配对规则读取正文。
...
`ProjectConversation.Generation` 初始为 0，清空时在事务内递增，同时删除历史、trace、checkpoint、ProjectConversationBinding 和 AgentSessionState，保留 ID、ContextId、标题和用量。InProcess 执行与清空共享 conversation execution lease；Durable 的 Queued、Running、WaitingForHuman 状态阻止清空。下一轮重新读取代次，释放旧 runtime 和解析任务缓存；Resume 指向已清空会话时创建新代次的任务。
...
部署与隔离 PostgreSQL 验证见 [执行持久化说明](Persistence/README.md)。
...
内置 Skill 可通过 `IAgentSkillRegistration.Tools` 携带独立具名工具。Execution 仅为已绑定 Skill 装配工具，绑定当前 Project，按工具声明处理权限和 Plan 模式；`agw-job` 的五个操作使用 `agw_job_*` 工具，原五个脚本入口退役。
...
Claude Code External Agent 默认启用 SDK partial messages。适配器读取原生 `StreamEvent` 的内容块索引，SDK 后续补充的工具内容继续追加到同一条消息。Codex 按 item ID 保持消息身份，文本、thinking 和 System 内容按收到的顺序追加。Pi 输出文本增量和新增内容块，每个内容只通过流式路径交付一次。usage 继续走计量链路，Result 使用独立消息 ID。

实时消息通过 `ReceiveMessage(AgwMessage)` 追加内容。`additionalProperties` 携带 `sourceMessageId`、`producerScopeId`、`conversationGeneration` 等来源信息；`@agw/execution-core` 按消息和内容块身份累积正文。服务器在持久化边界读取完整历史快照，SDK 流式历史回调只参与调用收尾。取消或失败时保存已经收到的正文，后续模型历史继续保留这些正文，并过滤未配对的工具调用。执行状态由 turn 和 execution 管理。

扩展 Agent 时提供适配器 factory，将原生新增内容及其消息、内容块身份交给公共追加路径，并使用 SDK 消息处理器和真实数据库验证交付与历史读取。接入说明见 [History](Agents/History/README.md)。
...
- 明文 `ProjectId` / `ProjectConversationId` 与 `ScopeBackfilled`，用于项目/会话范围索引和调度资格校验；
...
`DurableExecutionScopeMaintenance` 保留 execution 锁下的归属一致性校验、会话 generation 校验及损坏隔离。新执行同时写入 Project/Conversation 归属及 `ScopeBackfilled=true`；调度只领取索引完整且标志为 true 的记录。旧清单缺少用户或工作区快照时拒绝恢复，不回退管理员、不重新捕获目录。启动、后台、删除及 checkpoint 恢复均不再回填历史记录。

损坏记录在 execution 锁和 StateVersion 条件更新保护下隔离，非终态转为 Failed；并发 Interrupt 胜出时不覆盖。明确与 Manifest 冲突的索引归属会清空；无法解密但既有索引未被否定时保留归属以支持清理。原始加密数据保留。未回填记录不会被标记为已回填，也不会根据猜测的归属清理。段启动仍复用一次加载/解密的健康记录，物化失败时额外读取标量元数据用于隔离。
...
| --- | --- | --- |
| PostgreSQL（默认） | 不增加基础设施；消息与状态使用同一个共享数据库 | 默认按 250 毫秒或 100 条事件批量提交；每条事件仍独立存储，需要制定表清理策略 |
...
在 `Agw.Agents.Execution.DependencyInjection.AddAgentExecution` 中增加：
