...
public sealed partial class BackendArchitectureTests
{
    ...
    [Fact]
    public void ModuleSource_RawPersistenceAccessMatchesExactInventory()
    {
        // Arrange
        var serverRoot = GetServerRoot();
        ...
        // Act
        var violations = FindRawPersistenceViolations(sources, RawPersistenceAccessInventory);

        // Assert
        Assert.Empty(violations);
    }

    private static string[] FindRawPersistenceViolations(
        IReadOnlyDictionary<string, string> sources,
        IReadOnlyDictionary<string, (int DbContext, int Repository)> expected
    )
    {
        var actual = sources
            .Where(pair => ShouldScanRawPersistenceSource(pair.Key, pair.Value))
            .ToDictionary(pair => pair.Key, pair => CountRawPersistenceAccess(pair.Value), StringComparer.Ordinal)
            .Where(pair => pair.Value != (0, 0))
            .ToDictionary(pair => pair.Key, pair => pair.Value, StringComparer.Ordinal);
        var violations = actual
            .Keys.Union(expected.Keys, StringComparer.Ordinal)
            .Where(path => !actual.ContainsKey(path) || !expected.ContainsKey(path) || actual[path] != expected[path])
            .Select(path =>
                $"{path}: expected {expected.GetValueOrDefault(path)}, actual {actual.GetValueOrDefault(path)}. Update the exact inventory when repaying persistence debt."
            )
            .ToList();

        foreach (var (path, source) in sources.Where(pair => ShouldScanRawPersistenceSource(pair.Key, pair.Value)))
        {
            foreach (Match alias in PersistenceTypeAliasRegex().Matches(source))
            {
                if (RawPersistenceAccessRegex().IsMatch(alias.Groups["target"].Value))
                {
                    violations.Add(
                        $"{path}: raw persistence alias '{alias.Groups["alias"].Value}' hides the concrete dependency."
                    );
                }
            }
        }
    ...
    private static (int DbContext, int Repository) CountRawPersistenceAccess(string source)
    {
        var matches = RawPersistenceAccessRegex().Matches(source);
        return (
            matches.Count(match => match.Groups["context"].Success),
            matches.Count(match => match.Groups["repository"].Success)
        );
    }

    private static bool ShouldScanRawPersistenceSource(string path, string source) =>
    ...
    public void UserScopeFilter_Bypass_IsRestrictedToApprovedInfrastructurePaths()
    {
        ...
        var allowed = new HashSet<string>(
            [
                "Agw.Infrastructure/Auth/EfApiTokenCredentialReader.cs",
    ...
    [GeneratedRegex(@"(?<context>\b(?:AgwDbContext|DbContext)\b)|(?<repository>\bIRepository\s*<)")]
    private static partial Regex RawPersistenceAccessRegex();

    [GeneratedRegex(@"\busing\s+(?<alias>[A-Za-z_][A-Za-z0-9_]*)\s*=\s*(?<target>[^;]+);")]
    private static partial Regex PersistenceTypeAliasRegex();
