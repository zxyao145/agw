...
可以使用 Docker，或下载与操作系统及处理器架构匹配的 Portable Server。两种方式都包含 Web 界面；下面先完成本机访问，再说明目录挂载和远程访问。
...
打开 `http://localhost:30816/setup` 完成初始化。镜像包含静态 Web，浏览器直接访问 Server 即可。示例的 `latest` 适用于试用；持续运行的部署应换成 Releases 对应的固定版本标签。
...
Windows 使用 `agw-server.exe serve`。默认监听 `http://127.0.0.1:30816`，需要调整时使用 `ASPNETCORE_URLS`。先验证本机 Setup、Web 和一次对话，再配置远程访问。
