...
本文描述对外契约与预期行为。若实现或测试与本文冲突，应修正漂移，不能把临时偏差反向写成正式契约。
...
2. System Agent 显示 Agent 直接配置的 skills/tools，并将直接配置的 ToolBlocks 展开为具体成员 Tools；存在当前 Project 时再合并 Project 直接配置的能力。
...
- System 候选项不包含 Connection、MCP server 或其他运行时派生 tools。
...
flowchart LR
    ...
    Mode -->|"system"| SystemItems["Agent + optional Project skills/tools + ToolBlock members"]
    ...
    Input["UserInput token at caret"] --> Trigger{"/ or @"}
    ...
    SuggestionList --> Replace["Replace caret fragment and restore caret"]
```
...
`text` 是选择后写入输入框的完整文本。命令使用 `/name`，文件使用 `@relative/path`；路径含空格时使用 `@"relative/path with spaces"`。`description` 用于补充说明，`kind` 主要用于 Skill/Tool 徽标。
...
|---|---|---:|---:|
| System Agent，有 Project | Project + Agent 直接 skills/tools + ToolBlock 成员 | 是 | 可用 |
| System Agent，无 Project | Agent 直接 skills/tools + ToolBlock 成员 | 是 | 无 Project 时不可用 |
...
External Agent 的分流使用持久化 Agent 的 `Type` 与 `ExternalAgentKind`。`Type == External` 且 `ExternalAgentKind == ClaudeCode` 时返回 `claudeCode`；Codex、Pi 和其他 External Agent 返回 `unsupported`。Agent 的名称或显示名称不决定 SDK 类型。
...
路由不使用 path parameters。控制器通过 `ApiResult.Ok(...)` 直接返回 Bens.Results envelope，Web 的 `apiGet()` 在页面拿到数据前统一解包。下面的示例是解包后的 `data`：
...
```json
{
  ...
  "suggestions": [
    ...
    {
      "text": "/todos_add",
      "description": "Tool · Todo · Tracks multi-step work with a persistent todo list.",
      "kind": "tool"
    }
  ]
}
...
`AgentSuggestionAppService` 通过 `IAgentsDbContext` 先加载当前用户的 Agent 及 `AgentSkillRelations`。只有请求提供 `projectId` 时，才通过 `IProjectRuntimeFacade` 加载当前用户的 Project 快照及 Skill IDs。因此 Agent 与显式指定的 Project 同时不存在时，优先返回 Agent 错误。
...
| External Agent | 若提供了 Project 则先验证其存在，随后按 `ExternalAgentKind` 返回 mode |
| tools 配置无效 | 抛出明确的配置错误，不静默丢弃 |
| Tool 或 ToolBlock 未注册 | 忽略，不向客户端暴露无效候选项 |
...
2. 丢弃 `Guid.Empty`。
3. 按 ID 去重。
4. 通过 `ISkillReferenceFacade` 批量取得当前用户可见的 Skill 描述。
5. 丢弃空白名称。
...
### 6.4 System Agent 的 Tool 与 ToolBlock 聚合

独立 Tool 和 ToolBlock 来自 `Agent.Tools`，并在请求提供 `projectId` 时合并
`Project.Tools`。两个属性都是 `ToolValueObject` 列表；外层通过 `kind`
区分独立 Tool 和 ToolBlock，内层 `definition` 再通过 `name` 映射到具体强类型配置，例如：
...
Agent 与 Project 按 `definition.name` 合并，同名时 Project Definition 覆盖
Agent Definition。Suggestions 对合并结果中的独立 Tool 调用
`ToolRegistryService.GetTool(name)` 并返回一个 Tool 建议项；对 ToolBlock 则读取
catalog descriptor 的 `MemberToolNames`，将每个成员展开为 `kind: "tool"` 的建议项。
例如 `todo` 返回 `/todos_add`、`/todos_complete` 等成员，`/todo` 本身不会进入响应。
只有 registry 中真实存在的 Tool 或 ToolBlock 才能贡献建议。未注册的 name 会被忽略；
无效 kind、错误 options 或重复配置会作为配置错误返回。
...
- 独立 Tools 按名称忽略大小写去重。
- ToolBlocks 按名称合并后再展开成员；registry 保证成员名在所有 ToolBlocks 中唯一，且不与独立 Tool 重名。
- Skill 与 Tool 不跨类别去重。

因此 `/todos_add` 可以同时存在 Skill 和 Tool 两项。最终结果先按 `text` 忽略大小写
排序，再按 `kind` 排序；同命令下 Skill 位于 Tool 之前。这保证候选顺序不依赖关系
加载或 HashSet 枚举顺序。
...
| ToolBlock 成员 Tool | 将 `Tool`、ToolBlock display name、ToolBlock description 用 ` · ` 连接 | `Tool · Todo · Tracks multi-step work` |
...
`IGNORED_COMMANDS` 的代码定义是过滤列表的单一事实来源，位于 `src/clients/packages/chat/src/lib/chat/search-command.ts`。列表包含需要专门 UI、会改变本地配置、退出登录、恢复会话或当前 Chat 不应直接暴露的 Claude 命令，例如 `/help`、`/init`、`/mcp`、`/model`、`/resume`、`/login` 和 `/logout`。
...
输入框根据当前光标位置识别触发 token。匹配规则作用于 `input.slice(0, caretIndex)`，等价于：
...
- `/deploy later` 在光标位于文本末尾时不触发；把光标放在 `/deploy` 后、空格前并继续编辑时会触发，并保留后面的 `later`；
- `user@example.com` 不触发，因为 `@` 前不是输入开头或空白。

触发器返回类型、去掉 `/` 或 `@` 后的 query，以及触发片段在原输入中的起止位置；结束位置就是当前光标。

### 10.2 选择后的替换

选择建议时只替换触发片段，光标前后的文本都保留：

```text
Please run /dep  ->  Please run /deploy␠
Open @src/a      ->  Open @src/app.ts␠
Please run /dep| later  ->  Please run /deploy |later
```

上例中的 `|` 表示光标。替换后会复用已有的空白分隔符；若后缀紧邻光标，则补一个空格。Textarea 重新获得焦点，光标移动到插入项及其分隔符之后，而不是强制移到全文末尾。
...
隐藏目录和 `node_modules` 默认不进入递归搜索。用户在 `@path` 中明确输入以 `.` 开头的隐藏目录段后，搜索会进入对应的隐藏路径并继续返回补全建议；例如 `@.github/w` 和 `@./.github/w` 都可返回 `@.github/workflows/`。未明确输入隐藏段的普通搜索仍跳过隐藏目录，`node_modules` 始终忽略。结果先显示目录，再按相对路径排序。API 默认 limit 为 10；遍历达到 limit 后停止，再对已收集结果排序，因此它不是对整棵树完成全量排序后的全局 Top 10。

前端将结果映射为：

```ts
{
  text: result.relativePath.includes(" ")
    ? `@"${result.relativePath}"`
    : `@${result.relativePath}`,
...
双引号包裹完整项目相对路径，`@` 保持在引号外。例如 `post/agw/Agw 介绍.md` 映射为 `@"post/agw/Agw 介绍.md"`；不含空格的路径保持原格式。

`fullPath` 同样是 Project 根目录内的规范相对路径，不暴露宿主机绝对路径。

目录保留尾部 `/`，用于区分目录，并让 `abc/` 这类查询同时命中目录本身及其后代。选择后仍遵循统一替换规则，在插入项与后缀之间保留或补充空白。
...
- Agent 始终通过 owner-scoped `IAgentsDbContext` 验证；请求提供 Project 时再经 Projects Facade 验证归属。
...
- 同名 Skill/Tool 同时保留；
- 错误 ToolValue 配置返回明确错误，未注册 Tool/ToolBlock 被过滤；
...
- 当前光标处的 `/`、`@` 触发，以及前后文本和光标位置保留；
- 手动输入隐藏 `@path` 时返回对应路径的文件建议，普通搜索仍过滤隐藏目录；
- 选择含空格的文件建议时，用双引号包裹完整路径并把光标移动到闭合引号之后；
...
| 隐藏路径 | 手动输入 `@./.github/w` | 显示 `@.github/workflows/` 及其匹配后代 |
| 空格路径 | 选择 `post/agw/Agw 介绍.md` | 插入 `@"post/agw/Agw 介绍.md" `，光标位于尾部空格后 |
| 前文保留 | 输入 `Please run /dep` 后选择 | 变为 `Please run /deploy ` |
| 中段光标 | 在 `Please run /dep later` 的 `/dep` 后继续输入并选择 | 变为 `Please run /deploy later`，光标位于 `later` 前 |
...
| API query gate / mode 转换 | `src/clients/packages/chat/src/lib/chat/agent-suggestions.ts` |
| 命令搜索 | `src/clients/packages/chat/src/lib/chat/search-command.ts` |
| Claude init 处理 | `src/clients/packages/chat/src/lib/chat/ai-message-handlers.ts` |
| 页面状态与 React Query | `src/clients/packages/chat/src/ui-web/pages/chat/chat-workspace.tsx` |
| 触发器解析 / 文本替换 | `src/clients/packages/chat/src/ui-web/components/message/suggestion-trigger.ts` |
| 通用输入与建议列表 | `src/clients/packages/chat/src/ui-web/components/message/user-input.tsx` |
| 文件建议适配 | `src/clients/packages/chat/src/lib/chat/search-file.ts` |
...
要加入 MCP、Connection 或其他派生 tools，必须先定义：
