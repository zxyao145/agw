...
Owns the unified Tool capability model. `ToolRegistryService` discovers stateless `IAgwTool`, `IContextualTool`, and attributed Tool declarations at Host startup, generates their catalog metadata, and validates their persisted definitions. A standalone Tool can be selected independently; a ToolBlock is an atomic group whose member Tools are selected and removed together. Every standalone Tool and ToolBlock member declares `AgwToolPermission`; the Agent execution boundary turns `Write` and `Execute` declarations into approval requests while `None` and `ReadOnly` run directly. `ToolContribution` carries contextual Tools, ToolBlock providers, permission metadata, warnings, and owned runtime resources.
