...
public sealed class ModeToolBlockTests
{
    ...
    public async Task MaterializeAsync_ModeSetRequiresHumanConfirmationAndModeGetDoesNot()
    {
        ...
        Assert.Equal(
            "plan",
            ReadStringResult(await materialized.ModeGet.InvokeAsync(
    ...
    public async Task ModeSet_BeforeHumanConfirmation_RemainsPendingThenChangesMode()
    {
        ...
        var pendingResult = materialized.ModeSet
        ...
        Assert.Equal(
            "execute",
            await materialized.ModeProvider.GetModeAsync(
    ...
    private static AIFunctionArguments CreateArguments(
        ...
        new(new Dictionary<string, object?> { ["mode"] = mode })
        {
            Services = services
        };
