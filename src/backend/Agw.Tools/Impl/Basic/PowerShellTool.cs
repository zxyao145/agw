...
internal class PowerShellTool : IAgwTool
{
    ...
    [
        ...
        (new Regex(@"\bClear-Disk\b", RegexOptions.IgnoreCase | RegexOptions.Compiled),
        ...
        (new Regex(@"\bStop-Computer\b|\bRestart-Computer\b",
            RegexOptions.IgnoreCase | RegexOptions.Compiled),
    ...
    public PowerShellToolResult Execute(PowerShellToolParams toolParams)
    {
        ...
        {
            throw new AgwException(ErrorCodes.CommandExecutionFailed,
        ...
        if (!completed)
        {
            ...
            throw new AgwException(ErrorCodes.CommandTimeout,
    ...
    private static List<string> CollectWarnings(string command)
    {
        ...
        if (InteractivePattern.IsMatch(command))
        {
            const string interactive =
                "Note: command appears interactive/blocking and may hang under -NonInteractive";
