...
- Keep shared Tool declarations and generated-tool contracts in `Agw.Tools.Abstractions` without Agw project, persistence, ASP.NET Core, or MAF runtime dependencies. `Agw.Tools.Generators` compiles attributed methods, schemas, and direct invocation delegates; partial Skill and ToolBlock declarations use `IAgwToolSet<T>` for generated membership and descriptor factories. Attributed Tools must not fall back to runtime method scanning or reflection invocation. General-purpose Tools belong to `Agw.Tools`; business Tools belong to their owner module’s `Application/Tools`. Skill-owned Tools are exposed only through bound `IAgentSkillRegistration.Tools` or generated `ToolTypes`, with runtime project binding, name validation, and the shared permission pipeline. The global catalog consumes the `Agw.Tools` generated module by default; additional generated containers require explicit `AddToolCatalogTypes` registration.
...
- `UserScopeFilter` is the named Infrastructure query filter for persisted owners and MUST fail closed when no user context is active. HTTP and execution contexts MUST establish a stable user ID; only token validation, seeding, and scheduler scans may use the restricted system scope. The approved Agent/Project deletion Infrastructure adapters may selectively bypass the named user filter for child cleanup tied to a verified surviving root owner, retaining soft-delete filtering. Fully orphaned rows require a separate maintenance audit; normal requests MUST NOT guess their owner.
...
- Auth owns local users, external identities, the transactional user-ID allocator, and Desktop login grants. New numeric User IDs begin at `10000`; administrator `1001` remains unchanged. Claims and owner contracts keep decimal-string IDs. Verified `(issuer, sub)` identifies an external account; email never merges accounts. The Auth Infrastructure identity adapter may bypass only `UserScopeFilter` for verified identity lookup, proof-bound one-time grant exchange, and expired-grant cleanup. Restore the user context after provisioning or credential issuance; ordinary module queries stay owner-scoped.
