...
持续授权保存在 Agw 自有的 `Agw.ToolApproval.Grants` session 状态中。`MafApprovalGrantAgent` 在 SDK 调用入口记录经过权限归一的授权，`MafSessionApprovalState` 在授权检查时比较工具名和语义 JSON 参数。有效期属于 session；后续 turn 的权限模式或版本变化会使旧授权失效；同一 execution 的恢复始终使用启动时的权限快照。
