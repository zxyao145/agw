...
`Agw.Agents.Execution` is the execution implementation assembly of the logical Agents module. It references `Agw.Agents` one-way for definition reads, shared `AgentflowTopology`, and persistence seams. Host explicitly composes `AddAgents` with `AddAgentExecution`; A2A and Jobs continue to use `Agw.Agents.Contracts`. Management requests for Mermaid use `IAgentflowMermaidProvider`, resolved to the same scoped runtime and retaining the existing workflow construction behavior.
...
Before reusing an Agent runtime, each new InProcess turn checks the current owner's Agent definition version (`UpdateTime ?? CreateTime`) as well as Project, normalized Context, and Generation. A definition edit rebuilds the runtime while retaining the conversation and external provider-session binding; it does not alter an active turn. The version belongs to the Agent row, so independent edits to a referenced Provider are not a separate invalidation trigger.
