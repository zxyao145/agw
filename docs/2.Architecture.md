...
Persisted Agentflows are Input-rooted directed graphs compiled into Microsoft Agent Framework workflows by `AgentflowWorkflowCompiler`. Supported node kinds are Input, Output, Agent, Workflow-as-Agent, PromptAdapter, ClearMessages, HumanGate, CheckpointMarker, Concurrent, Handoff, GroupChat, and Magentic blocks. ClearMessages discards the current upstream message payload while continuing the turn with empty input; it does not reset downstream Agent session history. Edge routing supports independent Direct predicates, multi-selection Fan Out predicates, ordered first-match Switch cases with an optional default, and Fan-in Barriers grouped by target. HumanGate edge predicates evaluate the latest approved human reply, while rejection stops the workflow. Cycles are allowed only when every cyclic component has a Switch branch that exits the cycle. When a cyclic Fan-in Barrier combines the one-shot Input with an in-cycle source, the runtime retains the initial Input in workflow checkpoint state and reuses it on later iterations; other out-of-cycle barrier sources are rejected. `ConfigJson.switchCaseOrder` stores Switch case order without a separate edge-group model. `AgentflowDomainService` validates graph structure, references, block membership, routing-strategy consistency, cycle safety, and Switch invariants before persistence.
...
- `AgwAiModel` - LLM model definition
