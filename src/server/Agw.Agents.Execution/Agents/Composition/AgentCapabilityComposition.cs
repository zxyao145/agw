...
public sealed class AgentCapabilityComposition : IAsyncDisposable
{
    ...
    {
        _tools = tools.ToList();
        _toolNames = tools.Select(tool => tool.Name).ToHashSet(StringComparer.OrdinalIgnoreCase);
        if (declaredToolNames != null)
        {
            _toolNames.UnionWith(declaredToolNames);
        }
    ...
    internal void AddSkillTools(
        IAgentSkillRegistration registration,
        Guid projectId,
        AgwGeneratedToolCatalog? generatedToolCatalog = null
    )
    {
        var names = new HashSet<string>(_toolNames, StringComparer.OrdinalIgnoreCase)
        {
        ...
        var planModeNames = new List<string>();
        foreach (var declaration in registration.Tools)
        {
            AgwToolDeclarationValidator.Validate(declaration);
            if (!names.Add(declaration.Name))
            {
                throw new AgwException(
                    ErrorCodes.IntegrationToolNameConflict,
                    $"Skill '{registration.Name}' tool '{declaration.Name}' conflicts with another capability."
                );
            }

            var tool = declaration.ToAITool(projectId);
            if (!string.Equals(tool.Name, declaration.Name, StringComparison.Ordinal))
            {
                throw new AgwException(
                    ErrorCodes.InvalidParam,
                    $"Skill '{registration.Name}' declared '{declaration.Name}' but produced '{tool.Name}'."
                );
            }
            ...
            tools.Add(
                AgwToolMetadataBinding.Bind(
                    ...
                    new AgwToolMetadata(
                        ...
                        declaration.AllowInPlanMode
                    )
                )
            );
            if (declaration.AllowInPlanMode)
            {
                planModeNames.Add(tool.Name);
            }
        }

        foreach (Type toolType in registration.ToolTypes)
        {
            if (generatedToolCatalog == null)
            {
                throw new AgwException(
                    ErrorCodes.InvalidParam,
                    $"Skill '{registration.Name}' declares generated Tool type '{toolType.FullName}', but no generated Tool catalog is registered."
                );
            }

            IReadOnlyList<Agw.Tools.Abstractions.Generated.AgwGeneratedToolDescriptor> declarations =
                generatedToolCatalog.GetTools(toolType);
            if (declarations.Count == 0)
            {
                throw new AgwException(
                    ErrorCodes.InvalidParam,
                    $"Skill '{registration.Name}' Tool type '{toolType.FullName}' has no generated Tools."
                );
            }

            foreach (Agw.Tools.Abstractions.Generated.AgwGeneratedToolDescriptor declaration in declarations)
            {
                if (!names.Add(declaration.Name))
                {
                    throw new AgwException(
                ...
                AITool generatedTool = ToolRegistryService.BindGeneratedTool(
                    generatedToolCatalog.Create(declaration, projectId),
                    declaration,
                    $"skill:{registration.Name}"
                );
                tools.Add(generatedTool);
                if (declaration.AllowInPlanMode)
                {
                    planModeNames.Add(generatedTool.Name);
                }
            }
        }

        _tools.AddRange(tools);
        _toolNames.UnionWith(names);
        _planModeAllowedToolNames.UnionWith(planModeNames);
    }
