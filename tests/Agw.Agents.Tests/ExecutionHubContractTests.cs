...
public class ExecutionHubContractTests
{
    ...
    [Fact]
    public async Task DispatchBoundary_FilePathOutsideRoot_UsesStableHubError()
    {
        ...
        var action = new Func<Task>(() =>
            Task.FromException(
                new AgwException(ErrorCodes.FilePathOutsideRoot, "Path is outside the project workspace.")
            )
        );
        ...
        Assert.Equal(
            $"{ErrorCodes.FilePathOutsideRoot.Code}: Path is outside the project workspace.",
            exception.Message
        );
    }
}
