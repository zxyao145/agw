...
internal sealed class ModeStateSnapshotMiddleware
{
    ...
    )
    {
        ...
        foreach (var message in response.Messages)
        {
            ...
            rewrittenMessages.AddRange(
                await CreateSnapshotsAsync(message.Contents, session, callNames, snapshottedCallIds, cancellationToken)
            );
        }
    ...
    )
    {
        ...
        foreach (var result in contents.OfType<FunctionResultContent>())
        {
            ...
            snapshots.Add(
                await ToolStateSnapshots.CreateModeAsync(_provider, session, toolName, result.CallId, cancellationToken)
            );
        }
