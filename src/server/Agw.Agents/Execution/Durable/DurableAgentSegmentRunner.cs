...
internal sealed class DurableAgentSegmentRunner
{
    ...
    {
        var runtime = await _runtimeService.CreateDurableRuntimeAsync(
                manifest.AgentId,
                manifest.Task.ToProjection(),
                manifest.Settings.ToCommand(
        ...
        try
        {
            ...
            var messages = input.SegmentIndex == 0
        ...
        {
            failure = exception;
            await sink.WriteAsync(CreateFailureMessage(exception), CancellationToken.None)
                .ConfigureAwait(false);
    ...
    /// 创建不会再抛出运行时异常的失败分段结果。
    /// </summary>
    private static DurableExecutionSegmentResult Failure(
        Guid executionId,
        int segmentIndex,
        string error) =>
        new()
        ...
        IReadOnlyList<DurableResolvedInteraction> resolvedInteractions)
    {
        ...
        foreach (var resolved in resolvedInteractions)
        {
            ...
            contents.Add(ToolApprovalSupport.CreateResponse(
                approval,
