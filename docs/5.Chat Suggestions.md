...
`@` 文件建议没有这个问题。它只依赖当前 Project 的 workspace，与执行目标无关。因此 `/` 与 `@` 虽然使用同一个输入框，不能共用一套可用性判断。
...
- 文件服务负责把 workspace 文件树打平成规范相对路径，供 `@` 建议搜索。
...
flowchart LR
    ...
    Workspace["Project workspace"] --> FileSearch
...
| System Agent，无 Project | Agent 直接 skills/tools | 是 | 无 workspace 时不可用 |
...
| Agentflow | 无 | 否 | 可用 |
| 缺少 Agent ID | 无 | 否 | 有 workspace 时可用 |
...
- `@` 调用 `searchFile(workspace, query)`。

文件分支不读取 command source。Agentflow、Codex 或 suggestions API 失败都不会关闭 `@`。

workspace 为空时 `searchFile()` 直接返回空数组。请求失败时记录错误并返回空数组，不阻塞发送消息。
...
```http
GET /api/files/search?path={workspace}&keyword={query}&recursive=true
```
...
- 文件搜索的 `path` 先经过文件模块的路径校验，再访问文件系统。
...
| 文件搜索 API | `src/server/Agw.Files/Controllers/FilesController.cs` |
...
后端 `AgentSuggestionAppServiceTests` 当前 9 项中有 2 项失败；本次新增的无 Project Agent-only 聚合测试通过。剩余失败表明实现与本文规范仍有两处既有漂移：

1. `AgentSuggestionAppService` 当前没有在 description 前加入 `Skill` / `Tool`，空 Skill 描述也返回空字符串；规范要求自描述前缀。
2. `AgentSuggestionKind` 当前运行时序列化为 `Skill` / `Tool`，而 OpenAPI、TypeScript 和本文契约要求 `skill` / `tool`。

这两项属于实现缺陷，不是设计变更。修复时应以本文件第 6.1、6.6 节和现有失败测试为验收标准。
