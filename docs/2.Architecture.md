...
Execution code is organized by capability. `HumanInteraction` groups shared approval contracts and policies with `InProcess` waiting and `Durable` response restoration. `Runtimes` owns shared lifecycle and provider-specific coordination, `Messaging` owns SignalR delivery and durable event streams, and `Persistence/Durable` owns durable state storage. Agent-specific runtimes, adapters, sessions, and runners live under `Agents`; workflow compilation, node context, runners, and checkpoints live under `Agentflows`. Directory and namespace changes preserve the existing public Contracts, DI lifetimes, execution protocol, session keys, and persisted JSON formats.
