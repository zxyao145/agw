...
public class BashToolTests
{
    ...
    {
        var exception = Assert.Throws<AgwException>(() => new BashTool().Execute(new BashToolParams()));
    ...
    {
        var result = new BashTool().Execute(
            new BashToolParams
            {
    ...
    {
        var missingShell = Path.Combine(Path.GetTempPath(), $"agw-missing-shell-{Guid.NewGuid():N}");

        var result = new BashTool(missingShell).Execute(new BashToolParams { Command = "echo test" });
    ...
    public void Execute_Timeout_ReturnsCommandTimeoutResult()
    {
        var result = new BashTool().Execute(
            new BashToolParams
            {
                Command = OperatingSystem.IsWindows() ? "ping 127.0.0.1 -n 6 > nul" : "sleep 5",
                Timeout = 50,
            }
        );
