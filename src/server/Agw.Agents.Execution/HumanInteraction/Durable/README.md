...
- manifest，包括本轮权限快照和独立的下轮权限模式/版本；
- session / checkpoint；
...
提交命令校验 execution 所有者、session generation、交互 ID 和响应变体，按 execution 启动时的权限快照归一后保存；当前边界的全部响应到齐后进入 `Resuming`。重复提交同一已接受决定是幂等的，冲突回答被拒绝。
...
`SetPermissionModeCommand` 仅更新 manifest 的 `NextPermissionMode` / `NextPermissionVersion`。本轮 `PermissionMode` / `PermissionVersion` 不变；切换 FullAccess 不会完成已有工具审批，也不会代答用户输入或 HumanGate。响应接受、后续分段和恢复后的 SDK 授权检查都使用本轮快照。

权限选择通过短时控制锁与并发保存协调；Running 期间不改变 Worker 的 ownership version。`permission-status` 分别投影本轮与下轮权限，下一次显式 turn 才采用新选择。

## 格式与边界

类型化交互快照不包含旧消息兼容或旧 pending 迁移。启动 manifest 的项目与所有者格式保持 schema v1，settings 包含本轮和下轮权限版本，避免影响原有数据的归属校验和清理。旧 pending 不承诺恢复；其他业务数据不清理，也没有数据库结构变更。外部 Agent 不支持 Durable 执行。
