...
`@agw/web`, `@agw/desktop`, and `@agw/desktop-contracts` are packages in the pnpm Workspace rooted at `src/clients`, with tasks orchestrated by Turborepo. The Expo mobile client remains a separate npm workspace.
...
Desktop-specific renderer composition lives in `src/features/desktop/`. The Web application remains Electron's renderer and consumes browser-safe contracts from `@agw/desktop-contracts` rather than maintaining its own bridge and runtime data shapes.

# Desktop Client (`src/clients/desktop`)

The Electron main process at `src/main/index.ts` is a narrow platform adapter around the exported Web client, and `src/preload/index.ts` exposes the whitelisted bridge. The main process owns the custom `agw://app` protocol, OS credential encryption, daemon registration, tray/window behavior, and installer integration. The renderer has context isolation, sandboxing, and no Node.js integration.

`@agw/desktop` and `@agw/web` both depend on the framework-free `@agw/desktop-contracts` package for bridge, runtime, settings, server-profile, and execution-status contracts. Electron-only I/O and Web-only React/API composition remain in their application packages.
