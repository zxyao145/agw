...
public sealed class UserMemoriesController : ControllerBase
{
    ...
    )
    {
        ...
        return ApiResult.Ok(
            new PagedResult<UserMemorySummaryResponse>
            {
    ...
    public async Task<IActionResult> GetAsync([FromQuery] Guid id, CancellationToken cancellationToken = default)
    {
        var memory = await _appService.GetAsync(id, cancellationToken);
        return memory == null ? ErrorCodes.ResourceNotFound.ToApiResult() : ApiResult.Ok(MapDetails(memory));
    }
    ...
    private static UserMemorySummaryResponse MapSummary(UserMemorySummary memory) =>
        new(memory.Id, memory.Name, memory.Description, memory.CreateTime, memory.UpdateTime);
